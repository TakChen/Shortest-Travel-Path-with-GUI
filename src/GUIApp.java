...
public class GUIApp extends JFrame {
	...
	private class MapPanel extends JPanel {
		...
		public MapPanel(Dijkstra dijkstra) {
			...
			// initialize the array of node colors
			// used to draw nodes
			if (graph != null) {
		...
		/**
		 * Displays the graph: nodes as circles, edges as lines connecting them.
		 */
		public void drawGraph(Graphics g) {
			Iterator<Point> iteratorOverNodeLocations = graph.getNodeLocations();
			if (iteratorOverNodeLocations == null || nodeColors == null || nodeLabels == null) {
				System.out.println("Can't display nodes - the iterator over nodes is null");
				return;
			}
			int k = 0;
			while (iteratorOverNodeLocations.hasNext()) {
				Point p = iteratorOverNodeLocations.next();
		...
		public void drawPath(Graphics g, Color col) {
			...
			for (int k = 1; k < path.size(); k++) {
				...
				Point p1 = graph.getNode(vPrev).getLocation();
				Point p2 = graph.getNode(vCurr).getLocation();
		...
		public void changeColor(Point p, Color col) {
			...
			Iterator<Point> iteratorOverNodeLocations = graph.getNodeLocations();
			if (iteratorOverNodeLocations == null) {
				System.out.println("Can't change color - the iterator over nodes is null");
				return;
			}
			int k = 0;
			while (iteratorOverNodeLocations.hasNext()) {
				Point curr = iteratorOverNodeLocations.next();
